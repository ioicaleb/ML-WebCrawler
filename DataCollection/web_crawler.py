...
from DataCollection.sheet_manager import get_defunct_players as gdp
from DataCollection.export_manager import export_rounds
from DataCollection.login_manager import login, use_previous_login
from DataCollection.json_manager import *

def get_avatar_url(div, player_name):
    avatars = read_json("avatars") 
    avatar_url = div.select_one("div[class*='rank-'] > :first-child > :first-child > :nth-child(2) > img").get("src")
    if avatars and any(avatar.get("name") == player_name for avatar in avatars):
        for avatar in avatars:
            if avatar.get("name") == player_name and avatar["url"] != avatar_url:
                avatar["url"] = avatar_url
                avatars = sorted(avatars, key= lambda x: x["name"])
                write_json("avatars", avatars)
    else:
        new_avatar = {
            "name": player_name,
            "url": avatar_url
...
def get_round_results(driver, config):
    ...
    for div in divs:
        try:
            ...
            # Get player avatar
            dps = read_json("defunct_players")
            skip_avatar = False
            for dp in dps:
                if player_name == dp["name"]:
                    skip_avatar = True
                    break
            if not skip_avatar:
